...
const requestHospitalisation = axios.create({
...
requestHospitalisation.interceptors.request.use(
    async (config) => {
      const user = getUser();
  ...
  requestHospitalisation.interceptors.response.use(
    (response) => {
      return response;
    },
    async (error) => {
      ...
      if (error.response.status === 401 && !originalConfig._retry) {
        ...
        try {
          ...
          setUser(user)
          return requestHospitalisation(originalConfig);
        } catch (e) {
          deleteUser()
...
export default requestHospitalisation